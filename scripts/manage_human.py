...
def human_lidar_callback(msg, human):
    ...
    odom_lidar.pose.covariance[0] = 0.1
    odom_lidar.pose.covariance[7] = 0.1
    odom_lidar.pose.covariance[14] = 0.1
    odom_lidar.pose.covariance[21] = 0.1
    odom_lidar.pose.covariance[28] = 0.1
    odom_lidar.pose.covariance[35] = 0.3
...
def main():
    rospy.init_node("manage_human", anonymous=True)
    rate = rospy.Rate(10)
    ...
    if rospy.has_param("human_list"):
        ...
        while not rospy.is_shutdown():
            for human in retrieved_list:
                # Assuming there is a human_msg message for each human in retrieved_list
                human_msgs = rospy.wait_for_message(
                    "/" + human + "_position_camera", human_msg
                )
